...
{ 
	...
	while(fgets(buffer,1024,fp)!=NULL){
		sscanf(buffer,"%d %d %d ",&size,&resident,&shared);//공백 기준으로 3번째 숫자이용.
		printf("%d\n",shared);//공유메모리
...
int main(int argc, char* argv[]){
	int pid = -1;
	int dur = 10; //default값 :10초
	int pidexist=-1;
	
	if(is_pidndur_ok(argc, argv, &pid, &dur)==-1){return -1;}

	char spid[10];//입력받은 pid 값
	sprintf(spid, "%d", pid);
	
	for(int time=0; time<dur*2; time++){
		pidexist=get_sharedinfo(spid);
		if(pidexist == -1){printf("%s isn't exist.",spid);break;}//pid 존재 여부 판단
